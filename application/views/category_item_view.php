	...
	<div class="row">
		<div class="col-md-2">
			<div class="list-group">
				<a href="#" class="list-group-item" v-for="(category, index) in categories" v-bind:class="{ active: active_index === index }" v-on:click="set_active(index, category.id)">{{ category.name }} <i v-if="category.active == 0" class="pull-right text-red fa fa-ban" style="padding-top: 4px;"></i></a>
			</div>
		</div>
		<div class="col-md-6">
			<div class="box box-danger">
				<div class="box-header with-border">
					<div class="pull-right box-tools">
						<button type="button" class="btn btn-danger btn-sm  pull-right" v-on:click="new_item" v-if="active_category_id != 15">
							 Add Item
						</button>
					</div>
					<h3 class="box-title">
				...
				<div class="box-body">
					<div class="table-responsive" style="min-height: 520px;max-height: 520px;overflow-y: scroll;overflow-x: hidden;">
					...
					<form action="/action_page.php">
						...
						<div class="form-group">
							...
							<select required v-model="new_item_category" class="form-control selectpicker">
								<option></option>
								<option v-for="category in categories" v-if="category.id != 15" v-bind:value="{ category_id: category.id }">{{ category.name }}</option>
							</select>
						</div>
					</form>
				</div>
					...
					<form action="/action_page.php">
						...
						<div class="form-group">
							...
							<select required v-model="update_item_category" class="form-control selectpicker">
								<option></option>
								<option v-for="category in categories" v-if="category.id != 15" v-bind:value="category.id" >{{ category.name }}</option>
							</select>
						</div>
					</form>
				</div>
